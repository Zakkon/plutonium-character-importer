...
//#region Charactermancer Source Selector
...
class PageFilterSourcesRaw extends PageFilterClassesRaw{
...
class ModalFilterSources extends ModalFilter {
    ...
    async pPreloadHidden($modalInner) {
        ...
        if (this._filterCache) {
            ...
            console.log("NUM SOURCES", allSources.length);
            allSources.forEach((it,i)=>{
                if(i<10){
                    const listSelectClickHandler = new ListSelectClickHandler({list});
                    const listItem = ModalFilterSources.getListItem({listSelectClickHandler: listSelectClickHandler, pageFilter: pageFilter, src:it, srcI:i});
