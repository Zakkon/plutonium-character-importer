...
class BrewDoc {
    ...
    static isOperationPermitted_moveToEditable({brew, isAllowLocal=false}={}) {
        return !brew.head.isEditable && (isAllowLocal || !brew.head.isLocal);
    }

    mutMerge({json, isLazy=false}) {
        this.body = this.constructor.mergeObjects({
            isCopy: !isLazy,
            isMutMakeCompatible: false
        }, this.body, json);
        this.head.mutMerge({
            json,
            body: this.body,
    ...
    static mergeObjects({isCopy=true, isMutMakeCompatible=true}={}, ...jsons) {
        ...
        jsons.forEach(json=>{
            ...
            if (isMutMakeCompatible)
                this._mergeObjects_mutMakeCompatible(json);

            Object.entries(json).forEach(([prop,val])=>{
                switch (prop) {
                case "_meta":
                    return this._mergeObjects_key__meta({
                        out,
                        prop,
                        val
                    });
                default:
                    return this._mergeObjects_default({
                        out,
                        prop,
                        val
                    });
    ...
    static _META_KEYS_MERGEABLE_OBJECTS = ["skills", "senses", "spellSchools", "spellDistanceUnits", "optionalFeatureTypes", "psionicTypes", "currencyConversions", ];
    ...
    static _metaMerge_dependenciesIncludes(a, b) {
        if (a != null && b != null) {
            Object.entries(b).forEach(([prop,arr])=>a[prop] = [...(a[prop] || []), ...arr].unique());
            return a;
        }
    ...
    static _metaMerge_otherSources(a, b) {
        if (a != null && b != null) {
            Object.entries(b).forEach(([prop,obj])=>a[prop] = Object.assign(a[prop] || {}, obj));
            return a;
        }
    ...
    static _META_MERGE__STATUS_PRECEDENCE = ["invalid", "deprecated", "wip", "ready", ];

    static _metaMerge_status(a, b) {
        return [a || "ready", b || "ready"].sort((a,b)=>this._META_MERGE__STATUS_PRECEDENCE.indexOf(a) - this._META_MERGE__STATUS_PRECEDENCE.indexOf(b))[0];
    }
    ...
    static _mergeObjects_key__meta({out, val}) {
        ...
        Object.entries(val).forEach(([metaProp,metaVal])=>{
            ...
            if (!this._META_KEYS_MERGEABLE_OBJECTS.includes(metaProp))
                return;
    ...
    static _mergeObjects_default({out, prop, val}) {
        if (!(val instanceof Array))
            return out[prop] === undefined ? out[prop] = val : null;
    ...
    static _mergeObjects_mutMakeCompatible(json) {
        ...
        if (json.monster) {
            json.monster.forEach(mon=>{
                if (typeof mon.size === "string")
                    mon.size = [mon.size];

                if (mon.summonedBySpell && !mon.summonedBySpellLevel)
                    mon.summonedBySpellLevel = 1;
            }
            );
        }
...
class BrewDocHead {
    ...
    static fromValues({json, url=null, filename=null, isLocal=false, isEditable=false, }, ) {
        return new this({
