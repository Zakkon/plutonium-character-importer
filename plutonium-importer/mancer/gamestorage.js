import { SharedConsts } from "../veTools/config.js";
import { UtilGameSettings } from "../veTools/utils.js";
...
class GameStorage {
    ...
    static init() {
        UtilLibWrapper.addPatch("game.settings.get", this._lw_game_settings_get, UtilLibWrapper.LIBWRAPPER_MODE_WRAPPER, );

        game.settings.register(SharedConsts.MODULE_ID, this._STORAGE_KEY_RELOADER, {
            name: "-",
            scope: "world",
            config: false,
            default: 0,
    ...
    static setOverride(namespace, key, val) {
        MiscUtil.set(this._gameStorageValueOverrides, namespace, key, val);
    }
    static unsetOverride(namespace, key) {
        MiscUtil.deleteObjectPath(this._gameStorageValueOverrides, namespace, key);
    }
    ...
    static _register(key, {type="client"}={}) {
        ...
        game.settings.register(SharedConsts.MODULE_ID, keyProc, {
            name: keyProc,
            hint: keyProc,
            scope: type,
            config: false,
    ...
    static registerClient(key) {
        this._registerClient(key);
    }
    ...
    static async pSetClient(key, value) {
        const fullKey = this._registerClient(key);
        await game.settings.set(SharedConsts.MODULE_ID, fullKey, {
            _: value
        });
    }
    ...
    static async pSetWorldThenClient(key, value) {
        if (game.user.isGM)
            return this.pSetWorld(key, value);

        const existing = await this.pGetClientThenWorld(key);
        if (CollectionUtil.deepEquals(value, existing))
            return;
