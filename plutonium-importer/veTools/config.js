import {UtilGameSettings, UtilCompat, UtilPrePreInit, UtilHooks} from "./utils.js";
...
class Config {
  ...
  static _preInit_getLoadedConfig() {
    ...
    if (existingConfig == null || !Object.keys(existingConfig).length) {
      const conf = Config._getDefaultGmConfig();
      console.log("DEFAULT CONFIG:", conf);
      return { isLoaded: false, config: conf }; //Using the default gm config
    }
  ...
  static _getDefaultConfig(opts) {
    ...
    console.log("CONFIG_CONSTS", configConsts);
    const outputConfig = {};
    configConsts.forEach(([propName, propValue]) => {
      const obj = outputConfig[propName] = {};
      const func = input => Object.entries(input).forEach(([key, val]) => {
        if (isPlayer) {
          if (val.isPlayerEditable) { obj[key] = null; }
        }
        else { obj[key] = val["default"]; }
      });
      if (propValue.settings) { func(propValue.settings); }
      if (propValue.settingsAdvanced) { func(propValue.settingsAdvanced); }
      if (propValue.settingsHacks) { func(propValue.settingsHacks); }
    });
  ...
  static get(prop1, prop2, {isIgnorePlayer = false} = {}) {
    if (Config._CONFIG_TEMP[prop1]?.[prop2] !== undefined) {
      return Config._CONFIG_TEMP[prop1][prop2];
    }
    if (!UtilPrePreInit.isGM() && ConfigUtilsSettings.isPlayerEditable(prop1, prop2) && !isIgnorePlayer) {
      const val = (Config._CONFIG_PLAYER[prop1] || {})[prop2];
      if (ConfigUtilsSettings.isNullable(prop1, prop2) && val === null || val != null) {
        return this._get_getValidValue(prop1, prop2, val);
      }
    }
    const val = (Config._CONFIG[prop1] || {})[prop2];
    return this._get_getValidValue(prop1, prop2, val);
  }
